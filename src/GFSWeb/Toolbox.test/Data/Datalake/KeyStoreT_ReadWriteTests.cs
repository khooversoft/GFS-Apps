...
//public class KeyStoreT_ReadWriteTests
//{
//    private ITestOutputHelper _outputHelper;
//    private record TestRecord(string Name, int Age);

//    public KeyStoreT_ReadWriteTests(ITestOutputHelper outputHelper) => _outputHelper = outputHelper;
...
//                services.AddKeyStore<TestRecord>("file");
//            })
//            .Build();
...
//    private async Task<(IHost Host, IKeyStore<TestRecord> Store)> BuildTypedStore(bool useHash, bool useCache, [CallerMemberName] string function = "")
//    {
//        IHost host = await BuildService(useHash, useCache, function);
//        IKeyStore<TestRecord> keyStore = host.Services.GetRequiredService<IKeyStore<TestRecord>>();
//        return (host, keyStore);
//    }

//    private async Task<(IHost Host, IKeyStore Store)> BuildStore(bool useHash, bool useCache, [CallerMemberName] string function = "")
//    {
//        IHost host = await BuildService(useHash, useCache, function);
//        IKeyStore keyStore = host.Services.GetRequiredService<IKeyStore>();
//        return (host, keyStore);
...
//        (await keyStore.Get(path)).BeOk().Return().Be(testRecord);
...
//    {
//        var store = await BuildStore(useHash, useCache);
//        IKeyStore keyStore = store.Store;
...
//    {
//        var store = await BuildTypedStore(useHash, useCache);
//        IKeyStore<TestRecord> keyStore = store.Store;
...
//        (await keyStore.Get(path)).BeOk().Return().Be(testRecord1);

//        (await keyStore.Set(path, testRecord2)).BeOk();
//        (await keyStore.Get(path)).BeOk().Return().Be(testRecord2);
